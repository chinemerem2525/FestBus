...
searchByBusNumber(busNumber: number): string[] {
  // Convert the bus number to a string for comparison
  const busNumberStr = busNumber.toString();

  // Filter stops where the bus number starts with the specified bus number
  const stops = this.busStops
    .filter(busStop => busStop.busNumber.toString().startsWith(busNumberStr))
    .map(busStop => busStop.name);


  return stops.length > 1 ? stops : [
    `No stops found for bus number starting with ${busNumber}`
  ];
}
  ...
  getDistanceAndDuration(
    ...
    //Function to search for bus stops by bus number
    // searchByBusNumber(busNumber: number): BusStop[] {

    //   return this.busStops.filter(stop => stop.busNumber === busNumber);
    // }
}
